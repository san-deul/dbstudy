...
UPDATE DEPARTMENT_T
 ...
 WHERE DEPT_NO = 3;      -- WHERE절의 등호(=)는 동등비교 연산자 equal

-- SET절과 WHERE에서의 = 가 동작다름


-- 2. EMPOLYEE_T에서 부서번호(DEPART)가 2인 부서의 사원들의 기본급(SALARY)을 10% 인상하시오.
UPDATE EMPLOYEE_T
 ...
 -- 롤백
 ROLLBACK;
 --이전 커밋으로 돌아감.... 위에서는 UPDATE들이 취소됨...INSERT는 남아있음
...
-- 1. DEPARTMENT_T에서 부서번호(DEPT_NO)가 3인 부서를 삭제하시오.(부서번호가 3인 사원은 없다. 참조무결성에 영향을 미치지 않는다.)
DELETE 
  FROM DEPARTMENT_T 
 WHERE DEPT_NO = 3;
