/*
 ...
 3. 조인 종류
    ...
    2) 외부 조인 : 어느 한 테이블에만 존재하는 데이터를 조회대상에 포함
*/
...
/*
    ...
    3. ANSI 문법
        SELECT 조회할 칼럼, ...
          FROM 테이블1 INNER JOIN 테이블2  ((테이블 순서는 상관없긴하지만, PK를 가진 테이블을 왼쪽(앞)에 두는 게 성능상 좋음))
            ON 조인 조건
*/
...
SELECT D.DEPT_NAME, AVG(E.SALARY)
 ...
 /*
    ...
    2. 해당 테이블이 왼쪽에 있으면 왼쪽 외부조인이고 오른쪽에 있으면 오른쪽 외부조인이다.
    3. ANSI 문법
        1) 왼쪽 외부 조인
        SELECT 조회할 칼럼, ...
...
SELECT E.EMP_NO, E.NAME, D.DEPT_NAME
  FROM DEPARTMENT_T D RIGHT JOIN EMPLOYEE_T E -- 조회하고자 하는 정보가 오른쪽 테이블(EMPLOYEE_T)에 있으니 RIGHT 테이블
    ON D.DEPT_NO = E.DEPART;
